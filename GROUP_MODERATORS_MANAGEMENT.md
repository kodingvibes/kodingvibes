# Gestión de Moderadores en Grupos

## Descripción

Se ha implementado la funcionalidad para que los creadores de grupos puedan asignar y gestionar moderadores dentro de la página de administración del grupo.
...
- **`group_creator_can_update_member_roles`**: Permite al creador del grupo actualizar los roles de cualquier miembro
- **`group_creator_can_remove_members`**: Permite al creador del grupo eliminar cualquier miembro
...
2. **Estado Adicional**:
   ...
   - `isCreator`: Indica si el usuario actual es el creador del grupo

3. **Nuevas Funciones**:
   - `handleChangeRole()`: Permite cambiar el rol de un miembro (member, moderator, admin)
   - `handleRemoveMember()`: Permite remover un miembro del grupo

4. **Nueva Sección UI**: "Gestión de Moderadores"
   - Solo visible para el creador del grupo
   ...
   - Indicadores visuales para el creador del grupo
   - Información sobre cada rol

## Roles Disponibles

### 1. **Miembro** (`member`)
- Puede ver y crear posts en el grupo
- Puede comentar y votar

### 2. **Moderador** (`moderator`)
- Todos los permisos de miembro
- Puede gestionar tags del grupo
...
- No puede ser removido del grupo

## Reglas de Negocio

1. **Solo el creador puede cambiar roles**: Los moderadores y admins no pueden cambiar roles de otros miembros
2. **El creador no puede ser removido**: El sistema impide remover al creador del grupo
3. **Moderadores pueden remover miembros comunes**: Pero no pueden remover al creador ni cambiar roles
4. **Permisos de edición de tags**: Tanto moderadores como admins pueden gestionar los tags del grupo
...
6. **Remover miembro**: Usa el botón de papelera para remover un miembro del grupo
...
- Solo el creador del grupo puede actualizar roles (verificación en BD)
- Los moderadores pueden remover miembros pero no cambiar roles
- El creador del grupo no puede ser removido (verificación en BD y frontend)
...
- **Badge "Creador"**: Amarillo para identificar al propietario del grupo
...
3. **Como miembro regular**:
   ...
   - ✓ Ser redirigido al grupo principal
